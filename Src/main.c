...
void startUartCmdTask(const void* argument);


/* Buffer for receiving via UART */
...
void UART_SendCommand(const char* command) 
{
...
{
    UART_SendCommand("\033[2J"); // Efface l'écran et remet le curseur en position 1,1
    UART_SendCommand("\033[H");  // Optionnel: assure que le curseur est en haut à gauche

}

/* Initialize UART and display the menu */
void UART_InitMenu(void) {
  UART_ClearScreen();
  UART_SendString("Welcome to the UART Command Menu!\r\n");
  UART_SendString("Type 'help' to view available commands.\r\n");
}

/* Display the help menu */
void UART_DisplayHelp(void) {
  UART_SendString("\r\n--- Help Menu ---\r\n");
  UART_SendString("help : Displays this help menu.\r\n");
  UART_SendString("cmd1 : Execute command 1.\r\n");
  UART_SendString("cmd2 : Execute command 2.\r\n");
  UART_SendString("info : Display system information.\r\n");
}
...
void startUartCmdTask(void const * argument) {
  ...
  for(;;) {
    memset(uartBuffer, 0, sizeof(uartBuffer)); // Clear buffer at the start of each loop
    pos = 0;

    // Collect characters until a newline or carriage return
    while(HAL_UART_Receive(&huart2, &data, 1, 10) == HAL_OK) {
      if (data == '\n' || data == '\r') {
        uartBuffer[pos] = '\0'; // Null-terminate the string
        break; // Exit the inner loop on new line or carriage return
      } else if (pos < sizeof(uartBuffer) - 1) {
         uartBuffer[pos++] = data; // Store received data into buffer
      }
    }

    // Process the command
    // if (strcmp((const char*)uartBuffer, "help") == 0) {
    //   UART_DisplayHelp();
    // } else if (strcmp((const char*)uartBuffer, "cmd1") == 0) {
    //   UART_SendString("Executing Command 1...\r\n");
    // } else if (strcmp((const char*)uartBuffer, "cmd2") == 0) {
    //   UART_SendString("Executing Command 2...\r\n");
    // } else if (strcmp((const char*)uartBuffer, "info") == 0) {
    //   UART_SendString("System Information:\r\n");
    // } else {
    //   UART_SendString("Unknown command. Type 'help' for help.\r\n");
    // }
...
int main(void)
{
  ...
  /* Start Uart cmd task */
  osThreadDef(uartTaskCmd, startUartCmdTask, osPriorityNormal, 0, 128);
  osThreadCreate(osThread(uartTaskCmd), NULL);
